...
- 每个页面文档定义需要声明docType
- 需要定义语言编码(meta标签，http-equiv="content-Type" content="text/html;charset=utf-8")
- 正确使用注释，里边不要有虚线
- 标签元素和属性名字小写，这是为了利好大小写敏感的XHTML
- 特殊的字符用编码表示
- 图片要有alt属性
...
是否阻塞跟href和src没有关系，跟浏览器实现最佳体检的设计有关， img的加载不会阻塞html的解析，但img加载后并不渲染，它需要等待Render Tree生成完后才和Render Tree一起渲染出来。而JS被设计成阻塞。  
